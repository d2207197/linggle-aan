...
# vo_clusters_dic = pickle.loads(open( CLUSTER_ROOT_PATH +
# 'cluster_vo_large.pick','r').read())



## POS dictionary
logger.debug('Load bncwordlemma.pick')
# BNC_POS_Dic = pickle.loads(open( CLUSTER_ROOT_PATH +
# 'bncwordlemma.pick','r').read())


def BNC_POS(word):
    global mc
    mc.BNC.bncwordlemma.find_one({'word': 'play'}, {'_id': 0, 'lemmas': 1})


def BNC_first_POS(word, pos = "anv"):
    global mc
    pos = list(pos)
    mc.BNC.bncwordlemma.aggregate([{"$match": {'word': word}}, {"$unwind": "$lemmas"},{"$match":{'lemmas.POS': {'$in': pos} } } ,  {"$sort": {"lemmas.%": -1}}, {"$limit": 1}, {"$project": {"lemmas.POS": 1}}])['result'][0]['lemmas']['POS']

## clusters of objects for given VERB
logger.debug('Load cluster_vo_large.pick')
# vo_clusters_dic = pickle.loads(
    # open(CLUSTER_ROOT_PATH + 'cluster_vo_large.pick', 'r').read())

def vo_clusters(word):
    global mc
    return mc.cluster.VO_large.find_one({'word': word}, {'_id': 0, 'cluster': 1})['cluster']

## clusters of VERBs for given NOUN
logger.debug('Load cluster_ov_large.pick')
# ov_clusters_dic = pickle.loads(
    # open(CLUSTER_ROOT_PATH + 'cluster_ov_large.pick', 'r').read())
def ov_clusters(word):
    global mc
    return mc.cluster.OV_large.find_one({'word': word}, {'_id': 0, 'cluster': 1})['cluster']

## clusters of NOUN for given Adjective
logger.debug('Load cluster_an_nouns.pick')
# an_clusters_dic = pickle.loads(
    # open(CLUSTER_ROOT_PATH + 'cluster_an_nouns.pick', 'r').read())
def an_clusters(word):
    global mc
    return mc.cluster.AN_nouns.find_one({'word': word}, {'_id': 0, 'cluster': 1})['cluster']

## clusters of Adjective for given Noun
logger.debug('Load cluster_an_adj.pick')
# na_clusters_dic = pickle.loads(
    # open(CLUSTER_ROOT_PATH + 'cluster_an_adj.pick', 'r').read())

def na_clusters(word):
    global mc
    return mc.cluster.AN_adj.find_one({'word': word}, {'_id': 0, 'cluster': 1})['cluster']

## clusters of Subject for given Verb
logger.debug('Load cluster_vs_large.pick')
# vs_clusters_dic = pickle.loads(
    # open(CLUSTER_ROOT_PATH + 'cluster_vs_large.pick', 'r').read())
def vs_clusters(word):
    global mc
    return mc.cluster.VS_large.find_one({'word': word}, {'_id': 0, 'cluster': 1})['cluster']

## clusters of Verbs for given Subject
logger.debug('Load cluster_sv_large.pick')
# sv_clusters_dic = pickle.loads(
    # open(CLUSTER_ROOT_PATH + 'cluster_sv_large.pick', 'r').read())
def sv_clusters(word):
    global mc
    return mc.cluster.SV_large.find_one({'word': word}, {'_id': 0, 'cluster': 1})['cluster']

## clusters of Similar Adjecive
# logger.debug('Load new_syno_adj_scores.pick')
# adj_clusters_dic = pickle.loads(
#     open(CLUSTER_ROOT_PATH + 'new_syno_adj_scores.pick', 'r').read())

## clusters of Similar Noun
# logger.debug('Load new_syno_noun_scores.pick')
# noun_clusters_dic = pickle.loads(
#     open(CLUSTER_ROOT_PATH + 'new_syno_noun_scores.pick', 'r').read())

## clusters of Similar Verb
# logger.debug('Load new_syno_verb_scores.pick')
# verb_clusters_dic = pickle.loads(
#     open(CLUSTER_ROOT_PATH + 'new_syno_verb_scores.pick', 'r').read())


def adj_clusters(word):
    out = mc.synonyms.adj.find_one({"word": word}, {'_id':0 , 'synonyms_2level':1})
    if out: return out['synonyms_2level']
    else: return []

def noun_clusters(word):
    out = mc.synonyms.adj.find_one({"word": word}, {'_id':0 , 'synonyms_2level':1})
    if out: return out['synonyms_2level']
    else: return []
def verb_clusters(word):
    out = mc.synonyms.adj.find_one({"word": word}, {'_id':0 , 'synonyms_2level':1})
    if out: return out['synonyms_2level']
    else: return []
...
def APIquery(query):
    ...
    for i in range(len(Search_Result)):
        ...
        Return_Result.append((phrase.replace('<span class="SW">',
                             "").replace("</span>", ""), freq, percentage))
...
##        pass
    ...
    if len(query_in) == 0:
        Final_Result = [("old", [])]
        resp = Response(
            json.dumps(Final_Result), status=200, mimetype='application/json')
        return resp

    if len(query_in) == 1:
        if query_in[0].isalpha():  # 查一個字預設是查 similar word
            query_word = query_in[0]
            Final_Result = []
            Joanne_Sim_Map = {"a": adj_clusters, "v":
                                  verb_clusters, "n": noun_clusters}
            ##未查先猜詞性
            try:
                # POS_Candi = [data[1] for data in
                # BNC_POS_Dic[query_in[0].lower()] if data[1] in "avn"][0]
                POS_Candi = BNC_first_POS(query_in[0].lower(), "avn")
            except:  # 猜不到詞性 預設名詞
                POS_Candi = "n"

            Sim = Joanne_Sim_Map[POS_Candi]  # 使用對應的字典

            try:
                clusters = Sim(query_word)
            except:
                clusters = []
            ...
            Result = [{"count":0, "phrase":data[0], "percent":
                       str(data[1]).strip(), "count_str":""} for data in Result]
            ...
            resp = Response(json.dumps(
                Final_Result), status=200, mimetype='application/json')
            return resp
        else:
            Final_Result = [("old", [])]
    ...
    if len(query_in) == 2 and query_in[0].isalpha() and query_in[1] in ["$N", "$V"]:  # VERB $N or ADJ $N or Subj $V
        ...
        if query_in[1] == "$N":  # VERB $N or ADJ $N
            ...
            for data in collocates:
                collocates_dic[lemmatizer.lemmatize(
                    data[0][1], 'n')].append((data[0][1], data[1]))
                total_no += data[1]

            ##取得 cluster 狀況
            ##跟據首字最有可能的詞性進行搜尋
            # POS_Candi = [data[1] for data in BNC_POS_Dic[
            #     query_in[0].lower()] if data[1] in "av"][0]

            POS_Candi = BNC_first_POS(query_in[0].lower(), "av")
            ...
            if POS_Candi == "a":
                ...
                clusters = vo_clusters(query_in[0])
            ...
            for cluster in clusters:
                ...
                for sub_cluster in cluster:
                    Sub_Cluster_Details = [
                    ]  # 記錄目前處理的 sub-cluster (level 2)的內容
                    sub_cluster_cnt = 0.0
                    ...
                    for word in sub_cluster:
                        if len(collocates_dic[word]) > 0:
                            for collocate in collocates_dic[word]:
                                ...
                                cluster_wno += 1
                                Sub_Cluster_Details.append(
                                    (collocate[0], collocate[1]))
                                ##記錄Label
                    ...
                    if len(Sub_Cluster_Details) > 0:
                        ## 開始排序
                        Sub_Cluster_Details.sort(
                            key=lambda x: x[1], reverse=True)

                        ## 進行格式化
                        now_datas = {}
                        now_datas['count'] = sub_cluster_cnt
                        now_datas['percent'] = ConvertPercentage(
                            sub_cluster_cnt * 100 / total_no)
    ...
    # condition: $A_beach, $V_cultivate
        ...
        for data in collocates:
            ...
            # edited by Maxis
            # solve the issue: "leading car" occurs in the results of
            # "$V car"
        ...
        ##跟據第二個字最有可能的詞性進行搜尋
        # POS_Candi = [data[1] for data in BNC_POS_Dic[
            # query_in[1].lower()] if data[1] in "nva"][0]
        POS_Candi = BNC_first_POS(query_in[0].lower(), "avn")
